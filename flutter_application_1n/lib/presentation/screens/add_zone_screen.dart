...
class _AddZoneScreenState extends State<AddZoneScreen> {
  ...
  void _onSavePressed({
    ...
    if (_formKey.currentState!.validate()) {
      ...
      // 1. إغلاق شاشة المودال فوراً ليظهر الـ SnackBar في الشاشة الرئيسية بالأسفل
      Navigator.pop(context);

      if (!isEdit) {
        // 2. التقاط نص النتيجة الإدارية المرجعة من الدالة مباشرة في متغير محلي
        ...
        // 3. إظهار الـ SnackBar بناءً على النتيجة الراجعة الصافية لعزلها عن الـ status الحركي
        if (mounted) {
  ...
  // دالة موحدة ومطورة لعرض رسائل النظام التفاعلية (Floating UI Feedback)
      ...
      builder: (context, loc, _) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) => ZoneCard(
            zone: loc.safeZones[index],
            index: index,
            patientId: widget.patientId,
            locProvider: loc,
            onEdit: () => _openZoneModal(zone: loc.safeZones[index], index: index),
  ...
  Widget _buildTypeDropdown(String value, ValueChanged<String?> onChanged) {
    return DropdownButtonFormField<String>(
      ...
      items: _nameOptions.map((n) => DropdownMenuItem(value: n, child: Text(n))).toList(),
      onChanged: onChanged,
    );
  }
  ...
  Widget _buildAutoButton(TextEditingController lat, TextEditingController lng, StateSetter setModalState) {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: const Text("استخدام موقعي الحالي", style: TextStyle(fontFamily: 'Cairo')),
      ),
    );
  }
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.location_off_rounded, size: 80, color: Colors.grey[200]),
          const SizedBox(height: 15),
  ...
  void _confirmDeletion(int index) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Text("حذف المنطقة", style: TextStyle(fontFamily: 'Cairo')),
